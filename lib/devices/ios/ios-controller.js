  ...
  async.waterfall([
    ...
    function (response, cb) {
      ...
      async.until(
        ...
        function (cb) {
          setTimeout(function () {
            console.log('Checking for the screenshot file '+ shotPath);
            if (fs.existsSync(shotPath)) {
              console.log('Attempting to read the screenshot file');
              fs.readFile(shotPath, function (err, _data) {
                if (err) {
                  console.log('Error reading the screenshot file');
                  return cb(new Error("Error reading the screenshot file. " + err.toString()));
                }
                if (!err) {
              ...
              console.log('Too many retries ... bailing');
              return cb(new Error("Timed out waiting for screenshot file"));
            } else {
              // Clear UIAutomations buffer
              var clearCmd = "1+1";       //["au.capture('temp-clear-buffer')"].join('');
              this.proxy(clearCmd, function () {});
            }
